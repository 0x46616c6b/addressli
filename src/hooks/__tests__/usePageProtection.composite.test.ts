...
describe("usePageProtection", () => {
  ...
  it("should use default original title when not provided", () => {
    ...
    expect(mockUseProcessingTitle).toHaveBeenCalledWith(false, undefined, "addressli - CSV Geocoding Tool");
  });
  ...
  it("should pass progress to useProcessingTitle when provided", () => {
    ...
    expect(mockUseProcessingTitle).toHaveBeenCalledWith(true, progress, "addressli - CSV Geocoding Tool");
  });
});
