...
describe("csvParser utilities", () => {
  ...
  describe("validateColumnSelection", () => {
    ...
    it("should fail when no address columns are selected", () => {
      ...
      expect(result.errors).toContain("Mindestens eine Adresskomponente (PLZ, Straße oder Ort) muss ausgewählt werden");
    });

    it("should fail when headers are empty", () => {
      const result = validateColumnSelection([], "zipCode", "street", "city");
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Keine Spaltenüberschriften gefunden");
    });
    ...
    it("should fail when selected column does not exist in headers", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Ungültige Spalten ausgewählt: nonexistent");
    });
    ...
    it("should fail when multiple selected columns do not exist", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Ungültige Spalten ausgewählt: nonexistent1, nonexistent2");
    });
